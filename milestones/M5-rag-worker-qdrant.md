...
- 扩展 `.env.example`：
  ...
  - `EMBEDDING_MODEL`
  - `EMBEDDING_DIMENSION`
- 明确 embedding API 是否复用 `LLM_BASE_URL` 和 `LLM_API_KEY`。
...
| `embedding.go` | OpenAI Compatible embedding client |
...
- M5 已正式开启。
- 尚未实现 RAG 代码、migration 或前端入口。
- 下一步建议从 M5.0 开始：补 Qdrant 环境、确认 embedding 配置、设计 migration。
