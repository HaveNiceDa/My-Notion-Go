...
- 收藏分组：按 `is_starred` 展示收藏文档，支持后端持久化排序。
...
- M6.3 已完成：
  - 新增 `public_id` 作为公开链接 ID，不直接暴露内部文档 ID。
  - 新增发布 / 取消发布接口和匿名公开只读接口。
  - 顶部栏新增发布弹窗，支持复制公开链接。
  - 前端新增 `/p/:publicId` 公开页面路由，使用 BlockNote 只读渲染。
  - 公开接口只返回标题、图标、封面、正文和更新时间，不返回内部权限 / RAG / 树结构字段。
  - `smoke-documents-api.mjs` 覆盖发布、公开读取、取消发布后不可访问。

## 剩余待办

- 搜索质量增强：评估 PostgreSQL `pg_trgm` / `tsvector`，必要时再切 Elasticsearch。
- 搜索索引架构：如果引入 ES，需要补异步索引任务、权限 filter、删除清理和本地 smoke。
- 公开页 SEO：补 title/meta、分享卡片和更稳定的公开页 head 信息。
- 公开页体验：补品牌 footer、空正文态、移动端阅读细节和返回产品入口。
- 发布权限细化：当前只支持单篇公开，后续可评估子页面继承发布、发布范围和访问统计。
- 发布弹窗健壮性：补复制失败 fallback、toast 提示和浏览器 clipboard 兼容处理。
