...
- 已完成 M8.4 首批基础落地：
  ...
  - 已完成 M8.4.3 AI 体验补强：思考态改为气泡内 dots 动画，移除重复文字提示；打开历史会话、消息追加、键盘弹起/收起和输入框聚焦时自动滚动到底部。
  - 已补齐输入区体验：发送成功后保持输入焦点，多行输入限制最大高度，取消生成会清理未落库的 pending 用户消息和临时 assistant 气泡。
